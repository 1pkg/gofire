	...
	table := map[string]struct {
		...
		"complex nested type with valid value should be parsed properly": {
			...
			out: map[interface{}]interface{}{
				...
				"c d":  []interface{}{},
				"test": []interface{}{[]interface{}{int64(0), int64(0), int64(0)}, []interface{}{int64(1), int64(-1), int64(1)}},
			},
		},
	}
	for tname, tcase := range table {
		t.Run(tname, func(t *testing.T) {
			out, set, err := ParseTypeValue(tcase.typ, tcase.val)
			if fmt.Sprintf("%v", tcase.err) != fmt.Sprintf("%v", err) {
				t.Fatalf("expected error message %q but got %q", tcase.err, err)
			}
			if tcase.set != set {
				t.Fatalf("expected set value %t but got %t", tcase.set, set)
			}
